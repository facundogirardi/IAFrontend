...
export default function Usuario(props) {
  ...
  const [tipomovimiento, setTipomovimiento] = React.useState("Depósito por Cajero Automatico");
  ...
  const validarLogin = async function () {
    getUsuarioUsuario(window.localStorage.getItem('name')).then((value) => {
      ...
      if(numerico<1){ swal(" ", "NO SE PUEDE DEPOSITAR UN MONTO MENOR A $ 1", "error"); }else{
