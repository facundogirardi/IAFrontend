...
export default function () {
  ...
  const [usuariotipo, setUsuariotipo] = React.useState(2);
  ...
  const subirDatos = async function () {
    ...
    if (
      ...
      swal(
        " ",
        "VERIFICAR QUE LOS DATOS ESTÉN COMPLETADOS CORRECTAMENTE",
        "warning"
      );
    }
    return archivoDatos;
  ...
  const redirect = async () => {
    ...
    if (ok) {
      swal(" ", "EMPRESA REGISTRADA CORRECTAMENTE", "success");
      setTimeout(() => {
        history.push({
            ...
            <center>
              ...
              <form autoComplete="off">
                ...
                <TextField
                  ...
                  label="Empresa"
                  inputProps={{
                    onChange: (event) => handleEmpresa(event),
                  }}
                />{" "}
                <br />
                <br /> Fecha de Inicio de Actividad <br />
                <TextField
                  required
                  id="fecha"
                  type="date"
