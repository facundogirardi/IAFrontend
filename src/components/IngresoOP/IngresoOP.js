    ...
    if (getLogin.rdo === 3) {
      ...
      getUsuarioUsuario(usuario).then((value) => {
        if (value.length !== 0) {
          ...
          history.push({
            pathname: "/Extracciones", //paso el usuario temporalmente
          });
        } else {
          swal(" ", "USUARIO INEXISTENTE", "error");
          history.push({
            pathname: "/IngresoOP/", //paso el usuario temporalmente
          });
        }
      });
    }
